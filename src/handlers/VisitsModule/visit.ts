...
import { Visit, VisitStatus, VisitCompanion, Company, CompanyPerson, Department, Agent } from "../../models/VisitsModule";

const includeRelations = [
    { model: Company,       as: 'company',       attributes: ['id', 'name'] },
    {
        model: CompanyPerson, as: 'company_person',
        attributes: [
            'id', 'name', 'document_number', 'license_number',
            [literal("NULLIF(\"company_person\".\"document_photo_front\", '') IS NOT NULL"), 'has_document_photo_front'] as [ReturnType<typeof literal>, string],
            [literal("NULLIF(\"company_person\".\"license_photo\", '') IS NOT NULL"), 'has_license_photo'] as [ReturnType<typeof literal>, string],
        ],
    },
...
export const updateVisit = async (req: Request, res: Response) => {
    try {
        ...
        const transaction = await db.transaction()
        try {
            const updateData: Partial<Pick<Visit,
                'company_id' | 'company_person_id' | 'date' | 'department_id' | 'responsible_person' | 'destination'
            >> = {}
