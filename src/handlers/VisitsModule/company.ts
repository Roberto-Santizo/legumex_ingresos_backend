...
import { Company } from "../../models/VisitsModule";
import { Op, WhereOptions } from "sequelize";

export const createCompany = async (req: Request, res: Response) => {
    try {
        const company = await Company.create({ ...req.body, created_by: req.user!.name })
...
export const getCompanies = async (req: Request, res: Response) => {
    try {
        ...
        if (req.query.all === 'true') {
            const rows = await Company.findAll({ where, order: [['createdAt', 'ASC']] })
...
export const updateCompany = async (req: Request, res: Response) => {
    try {
        ...
        const { name } = req.body
        const updateData: Partial<Pick<Company, 'name'>> = {}
