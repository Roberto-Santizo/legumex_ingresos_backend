...
import { UniqueConstraintError, literal, Op, WhereOptions } from "sequelize";
import { CompanyPerson, Company } from "../../models/VisitsModule";
...
export const getCompanyPersons = async (req: Request, res: Response) => {
    try {
        ...
        const where: WhereOptions<CompanyPerson> = {}
        ...
        const { count, rows } = await CompanyPerson.findAndCountAll({
            ...
            attributes: {
                ...
                include: [
                    [literal("NULLIF(document_photo_front, '') IS NOT NULL"), 'has_document_photo_front'] as [ReturnType<typeof literal>, string],
                    [literal("NULLIF(document_photo_back, '') IS NOT NULL"), 'has_document_photo_back'] as [ReturnType<typeof literal>, string],
                    [literal("NULLIF(license_photo, '') IS NOT NULL"), 'has_license_photo'] as [ReturnType<typeof literal>, string],
                ],
            },
            limit,
            offset,
...
export const updateCompanyPerson = async (req: Request, res: Response) => {
    try {
        ...
        const updateData: Partial<Pick<CompanyPerson,
            'company_id' | 'name' | 'document_number' | 'document_photo_front' | 'document_photo_back' | 'license_number' | 'license_photo'
        >> = {}
